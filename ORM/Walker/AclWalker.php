...
use Pintushi\Bundle\SecurityBundle\Exception\NotFoundAclConditionFactorBuilderException;
use Pintushi\Bundle\SecurityBundle\ORM\Walker\Condition\AclConditionStorage;
use Pintushi\Bundle\SecurityBundle\ORM\Walker\Condition\JoinAssociationCondition;
use Pintushi\Bundle\SecurityBundle\ORM\Walker\Condition\SubRequestAclConditionStorage;
use Pintushi\Bundle\SecurityBundle\ORM\Walker\Condition\AclCondition;
...
{
    const APP_ACL_CONDITION = 'pintushi_acl.condition';
    const APP_ACL_FACTOR_BUILDER = 'pintushi_acl.factor.builder';
    ...
    public function walkSelectStatement(SelectStatement $AST)
    {
        ...
        if (!$query->hasHint(self::APP_ACL_FACTOR_BUILDER)
            || !$query->getHint(self::APP_ACL_FACTOR_BUILDER) instanceof AclConditionalFactorBuilder) {
            throw new NotFoundAclConditionFactorBuilderException();
        }

        $this->aclConditionFactorBuilder = $query->getHint(self::APP_ACL_FACTOR_BUILDER);

        if ($query->hasHint(self::APP_ACL_CONDITION)) {
            /** @var AclConditionStorage $aclCondition */
            $aclCondition = $query->getHint(self::APP_ACL_CONDITION);
